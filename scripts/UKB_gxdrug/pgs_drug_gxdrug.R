...
for (i in 1:length(drug_names)) {
  ...
  res[[i]] <- data.frame(PGS='pgs',drug=drug,hr=summary(mod)$coef["dataf.mg[, drug]",2], p=summary(mod)$coef["dataf.mg[, drug]",5])
}
results <- do.call(rbind,res)
# results <- results[1:60,]
results$fdr <- p.adjust(results$p,method = 'fdr')
results[order(results$p)[1:20],]
fwrite(results,'/athena/elementolab/scratch/anm2868/druggene/output/pgs_int/marginal_drug_assoc.txt',sep = '\t',quote = F,na = "NA",row.names = F,col.names = T)
